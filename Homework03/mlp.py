...
class SentimentClassifier(nn.Module):
    def __init__(self, embed_dim: int, num_classes: int, hidden_dims: List[int], activation: str = 'Sigmoid'):
        ...
        if activation == 'Sigmoid':
            ...
            print("Something went wrong initializing the sentiment classifier")
            return
    ...
    def forward(self, inp):
        ...
        # Apply MLP layers with activation for hidden layers
        for i, linear in enumerate(self.linears):
            temp = linear(temp)
            if i < len(self.linears) - 1:  # Apply activation except for the last layer
                temp = self.activation(temp)

        logits = temp  # Final layer outputs raw scores (logits)

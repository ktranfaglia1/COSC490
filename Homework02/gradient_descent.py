...
def create_tensor_dataset(raw_data: Dict[str, List[Union[int, str]]],
    ...
    for text, label in tqdm(zip(raw_data['text'], raw_data['label'])):
        ...
        # Add the feature and label to lists if it is not None
...
class SentimentClassifier(nn.Module):
    def __init__(self, embed_dim, num_classes):
        ...
        # Define the linear layer to map the input embedding to class scores
    ...
    def forward(self, inp):
        ...
        # Pass input tensor through the linear layer (gets raw output scores for classification)
    ...
    def softmax(logits):
        ...
        max_logits = torch.max(logits, dim=1, keepdim=True).values  # Get max along dim=1, keeping dimension
        exp_shifted = torch.exp(logits - max_logits)  # Compute exponentials after stabilization
        return exp_shifted / torch.sum(exp_shifted, dim=1, keepdim=True)  # Normalize
    ...
    def gradient_loss(self, inp, logits, labels):
        ...
        loss = -torch.mean(log_probs[torch.arange(bsz), labels])  # Cross-entropy loss

        # Compute gradients
        probs[torch.arange(bsz), labels] -= 1
        error = probs / bsz
        grads_weights = torch.matmul(error.T, inp)  # (num_classes, embed_dim)
        grads_bias = torch.sum(error, dim=0)  # (num_classes)

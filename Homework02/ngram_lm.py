...
def plot_next_word_prob(word_scores, word_candidates, context, top=10, save_path=None):
    ...
    # Get the scores and corresponding words
    scores = word_scores[context]
    words = word_candidates[context]
    
    # Sort words by probability in descending order
    sorted_indices = np.argsort(scores)[::-1]
    sorted_words = [words[i] for i in sorted_indices[:top]]
    sorted_scores = [scores[i] for i in sorted_indices[:top]]
    
    # Plot the probabilities
    plt.figure(figsize=(10, 5))
    plt.bar(sorted_words, sorted_scores, color='violet')
